...
// ============================================================
// APM MASTER — TENANT CONFIGURATION
// Edit this block before deploying to a new environment.
// Do not edit anything below the closing === line.
// ============================================================
const APM_CONFIG = {
...
// Dark mode early-paint (unchanged from original)
...
var mainWin, hostname, url, AppContext, FLAGS;
  ...
  // src/core/constants.js — MODIFIED
  // All hardcoded tenant/environment values replaced with _CFG references.
  // Storage key names, version strings, and UI constants are unchanged.
  var KEY_THEME, CC_STORAGE_RULES, CC_STORAGE_SET, PRESET_STORAGE_KEY, STORAGE_KEY,
      APM_GENERAL_STORAGE, CURRENT_VERSION, VERSION_CHECK_URL, UPDATE_URL,
      LABOR_EMPS_STORAGE, LABOR_ACTIVE_STORAGE, LABOR_DOCK_STORAGE, LABOR_PREFS_STORAGE,
      LABOR_NIGHT_SHIFT_KEY, LABOR_LAST_EMP_KEY, SESSION_STORAGE_KEY, PTP_HISTORY_KEY,
      UPDATE_CHECK_KEY, MIGRATIONS_DONE_KEY, WELCOME_SEEN_KEY, BETA_VERSION_CHECK_URL,
      BETA_UPDATE_URL, LOG_LEVELS, DEFAULT_TENANT, SESSION_TIMEOUT_URL, LINK_CONFIG,
      MIN_GRID_COLUMNS, MIN_TAB_ITEMS, ENTITY_REGISTRY, SCREEN_TITLES;
  ...
  var init_constants = __esm({
    "src/core/constants.js"() {
      ...
      // ── Update URLs — pulled from config ──────────────────────────────────
      VERSION_CHECK_URL   = _CFG.updateUrl;
      UPDATE_URL          = _CFG.updateUrl;
      BETA_VERSION_CHECK_URL = _CFG.updateUrl; // Override if you maintain a Beta branch
      BETA_UPDATE_URL        = _CFG.updateUrl;
      ...
      LOG_LEVELS = { ERROR: 0, WARN: 1, INFO: 2, DEBUG: 3, VERBOSE: 4 };

      // ── Tenant / environment ──────────────────────────────────────────────
      DEFAULT_TENANT     = _CFG.tenant;
      SESSION_TIMEOUT_URL = _CFG.sessionTimeoutUrl;

      // ── Link config — drives deep-link and email drillback behaviour ──────
      LINK_CONFIG = {
        tenant:       _CFG.tenant,
        userFuncName: _CFG.woScreenFunc,
        woPattern:    _CFG.woPattern,
      };

      MIN_GRID_COLUMNS = 3;
      MIN_TAB_ITEMS    = 2;

      // ── Entity registry — WO number pattern pulled from config ────────────
      // The pattern here governs which strings in grids/emails are recognised
      // as work order numbers and made clickable. Update WO_PATTERN in the
      // config block at the top if your WO numbers follow a different format.
      ENTITY_REGISTRY = {
        WSJOBS: {
          label:            "Work Order",
          systemFunc:       _CFG.woScreenFunc,
          entityKey:        "workordernum",
          dataIndex:        "workordernum",
          drillbackFlag:    "FROMEMAIL",
          pattern:          new RegExp(`(?:^|\\D)(${_CFG.woPattern.source})(?=\\D|$)`),
      ...
      SCREEN_TITLES = { BSSTRT: "Start Screen" };
    }
  });

  // ── Everything below this point is UNCHANGED from the original script ────
  // src/core/storage.js, recovery.js, api.js, diagnostics.js, logger.js,
  // ui/dom-helpers.js, core/toast.js, theme-resolver.js, state.js, ...
